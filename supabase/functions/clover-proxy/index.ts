...
// Wine family keywords (international + US-centric)
...
const WINE_FORMAT_KEYWORDS = [
  "bottle", "glass", "btg", "magnum", "jeroboam", "75cl", "375ml", "150cl",
...
serve(async (req) => {
  ...
  try {
    ...
    // Clover uses Bearer token auth
    // api_token stores the Bearer token, base_url stores e.g. https://api.eu.clover.com/v3/merchants/{mId}
    ...
    if (action === "test") {
      try {
        ...
        const res = await fetch(`${baseUrlClean}`, { headers });
        if (res.ok) {
    ...
    if (action === "find-last-business-day") {
      ...
      for (let i = 0; i < scanDays; i++) {
        ...
        // Build time range for the day in ms (Clover uses ms timestamps)
        ...
        try {
          ...
          if (res.ok) {
            ...
            if (orders.length > 0) {
              ...
              // Get actual count
              const total = body.href ? (body.elements?.length || orders.length) : orders.length;
              totalInvoicesFound += total;
            }
          }
        } catch (_) { /* skip */ }

        // Stop scanning after 10 consecutive empty days if we found some
        if (daysWithSales.length > 0) {
          const lastFound = daysWithSales[daysWithSales.length - 1];
          const daysSinceLastFound = Math.floor((Date.now() - new Date(lastFound).getTime()) / 86400000) - i;
          // Simple heuristic: break if gap > 10
        }
      }
    ...
    if (action === "fetch-day") {
      ...
      // Fetch orders with line items expanded
      let allOrders: any[] = [];
      let offset = 0;
      const limit = 100;
