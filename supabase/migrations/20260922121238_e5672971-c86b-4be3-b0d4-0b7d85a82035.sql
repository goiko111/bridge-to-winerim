CREATE OR REPLACE FUNCTION public.review_unmapped_products(p_connection_id uuid, p_search text DEFAULT NULL::text, p_family text DEFAULT NULL::text, p_format text DEFAULT NULL::text, p_status text DEFAULT NULL::text, p_days integer DEFAULT 30, p_limit integer DEFAULT 50, p_offset integer DEFAULT 0)
 RETURNS TABLE(provider_product_id text, provider_product_name text, family text, sale_format text, format_key text, units numeric, line_count integer, last_sale_at timestamp with time zone, agora_price numeric, decision_status text, selected_winerim_id text, selected_winerim_name text, selected_format_key text, force_ready boolean, note text, decided_at timestamp with time zone, total_count bigint)
 LANGUAGE sql
 STABLE
...
WITH sold AS (
  SELECT sli.provider_product_id pid, max(sli.name) pname, max(sli.family) pfamily, coalesce(sli.format,'') pformat, public.review_format_key(sli.format) fkey, sum(sli.quantity) units, count(*)::int line_count, max(sli.created_at) last_sale_at,
         (array_agg(sli.unit_price ORDER BY sli.created_at DESC NULLS LAST))[1] sold_price
  ...
  SELECT u.pid,coalesce(pp.name,u.pname) pname,coalesce(u.pfamily,pp.family) pfamily,u.pformat,u.fkey,u.units,u.line_count,u.last_sale_at,coalesce(pp.price, nullif(u.sold_price,0)) agora_price,coalesce(d.status,'DRAFT') decision_status,d.selected_winerim_id,d.selected_winerim_name,d.selected_format_key,coalesce(d.force_ready,false) force_ready,d.note,d.decided_at
