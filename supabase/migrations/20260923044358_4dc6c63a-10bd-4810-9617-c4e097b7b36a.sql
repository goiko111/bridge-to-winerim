...
WITH guard AS (
  ...
  SELECT DISTINCT ON (s.agora_product_id)
         s.agora_product_id AS pid, s.agora_price, s.winerim_wine_id, s.format_key
  ...
  SELECT prod ->> 'Id' AS pid,
         ...
         coalesce(nullif(public.review_format_key(prod ->> 'Name'), 'SIN_DATO'), snaps.format_key, 'SIN_DATO') AS sale_format,
         coalesce(nullif(public.review_format_key(prod ->> 'Name'), 'SIN_DATO'), snaps.format_key, 'SIN_DATO') AS fkey,
