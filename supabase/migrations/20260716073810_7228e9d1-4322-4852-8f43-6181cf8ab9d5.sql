...
begin
  ...
  return acquired_token = p_lock_token;
end;
$$;
...
begin
  if p_job not in ('catalog', 'sales-stock', 'outbound-queue', 'activation') then return false; end if;
  delete from public.agora_dispatch_locks where connection_id = p_connection_id and lock_token = p_lock_token;
