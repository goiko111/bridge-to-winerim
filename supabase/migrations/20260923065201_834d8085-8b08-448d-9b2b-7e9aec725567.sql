create index if not exists idx_product_mappings_conn_status on public.product_mappings (connection_id, status);
create index if not exists idx_sales_line_items_conn_created on public.sales_line_items (connection_id, created_at);
create index if not exists idx_winerim_wine_formats_conn_id_fmt on public.winerim_wine_formats (connection_id, winerim_id, format_key);

create or replace function public.review_mapped_products(p_connection_id uuid, p_search text DEFAULT NULL::text, p_format text DEFAULT NULL::text, p_state text DEFAULT NULL::text, p_days integer DEFAULT 30, p_limit integer DEFAULT 25, p_offset integer DEFAULT 0)
 returns table(provider_product_id text, provider_product_name text, format_key text, family text, agora_price numeric, winerim_wine_id text, winerim_wine_name text, winerim_price numeric, winerim_active boolean, match_method text, mapped_at timestamp with time zone, last_synced_at timestamp with time zone, units_recent numeric, mapped_state text, comparison text, total_count bigint)
 language sql
...
base as (
  select m.provider_product_id,
         ...
         upper(coalesce(nullif(m.format_type, ''), 'UNKNOWN')) as format_key,
         pp.family,
         ...
         coalesce(
           f.sale_price,
           case upper(coalesce(nullif(m.format_type, ''), 'UNKNOWN'))
             when 'BOTTLE' then w.bottle_sale_price
             when 'GLASS' then w.glass_sale_price
             when 'MAGNUM' then w.magnum_sale_price
             else null
           end
         ) as winerim_price,
  ...
  left join winerim_wine_formats f
    on f.connection_id = m.connection_id
   and f.winerim_id = m.winerim_wine_id
   and upper(f.format_key) = upper(coalesce(nullif(m.format_type, ''), 'UNKNOWN'))
...
filtered as (
  select c.*
  from classified c
  where (p_format is null or p_format = '' or c.format_key = p_format)
    and (p_state is null or p_state = '' or c.mapped_state = p_state)
    and (
...
create or replace function public.review_mapped_summary(p_connection_id uuid, p_days integer DEFAULT 30)
 returns table(mapped_total bigint, state_ok bigint, price_mismatch bigint, no_price_ref bigint, wine_inactive bigint, wine_missing bigint, bottle_count bigint, glass_count bigint, other_format_count bigint, units_recent numeric)
 language sql
 stable security definer
 set search_path to 'public'
as $function$
with rows as (
  select * from public.review_mapped_products(p_connection_id, null, null, null, p_days, 100000, 0)
)
select count(*),
