...
WITH pp_wine AS (
  ...
  CROSS JOIN LATERAL jsonb_array_elements(amd.products_summary_json) AS prod
  LEFT JOIN LATERAL (
    SELECT f ->> 'Name' AS fname
    FROM jsonb_array_elements(amd.families_json) AS f
    WHERE f ->> 'Id' = prod ->> 'FamilyId'
    LIMIT 1
  ) fam ON true
  ...
  SELECT m.pid,
         ...
         public.review_format_key(m.name) AS fkey,
         snap.agora_price AS price,
         m.visible_attr,
         m.saleable_attr,
         snap.winerim_wine_id
  FROM master_raw m
  LEFT JOIN LATERAL (
    SELECT s.agora_price, s.winerim_wine_id
    FROM public.catalog_readback_snapshots s
    WHERE s.connection_id = p_connection_id
      AND s.agora_product_id = m.pid
    ORDER BY s.read_at DESC
    LIMIT 1
  ) snap ON true
  WHERE public.review_is_wine(m.family, m.name, snap.winerim_wine_id)
), wine AS (
  SELECT * FROM pp_wine
  UNION ALL
