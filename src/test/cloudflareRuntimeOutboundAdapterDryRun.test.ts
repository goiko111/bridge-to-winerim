...
describe("PostgreSQL outbound adapter dry-run and retry persistence", () => {
  ...
  it("persists retry, breaker, backoff, sanitized log and exact tracking in one transaction", async () => {
    ...
    expect(persistedValues).toContain("failureDiagnostic");
    expect(persistedValues).toContain("outbound.transport");
    expect(persistedValues).toContain("OUTBOUND_TRANSPORT_THROW");
  });
});
