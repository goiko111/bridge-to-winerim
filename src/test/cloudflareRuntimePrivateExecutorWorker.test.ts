...
} from "../../cloudflare/workers/middleware-runtime-executor/src/agoraMasterRefresh";
import {
...
function successfulAgoraMasterRefresh(): AgoraMasterRefreshPort {
  return {
    refresh: vi.fn(async () => ({
      ok: true as const,
...
describe("private runtime executor Worker", () => {
  it("merges all configured fleet catalog profile chunks", () => {
    const profile = {
      v: "3",
      p: ["1"],
      w: ["1"],
      c: { B: "#8B0000", C: "#8B0000", M: "#8B0000" },
      t: "3",
      r: "2",
      od36: "",
      a: 1,
    };
    const profiles = fleetCatalogProfilesFromEnv({
      RUNTIME_FLEET_CATALOG_PROFILES_JSON: JSON.stringify({
        "11111111-1111-4111-8111-111111111111": profile,
      }),
      RUNTIME_FLEET_CATALOG_PROFILES_JSON_2: JSON.stringify({
        "22222222-2222-4222-8222-222222222222": profile,
      }),
      RUNTIME_FLEET_CATALOG_PROFILES_JSON_3: JSON.stringify({
        "33333333-3333-4333-8333-333333333333": profile,
      }),
      RUNTIME_FLEET_CATALOG_PROFILES_JSON_4: JSON.stringify({
        "44444444-4444-4444-8444-444444444444": profile,
      }),
    });
    ...
    expect(profiles?.[CONNECTION_ID]?.alwaysIncludeVintage).toBe(true);
  });

  it("fails closed for duplicate or invalid fleet catalog profile chunks", () => {
    const profile = {
      v: "3",
      p: ["1"],
      w: ["1"],
    ...
    const duplicate = JSON.stringify({ [CONNECTION_ID]: profile });

    expect(fleetCatalogProfilesFromEnv({
      RUNTIME_FLEET_CATALOG_PROFILES_JSON: duplicate,
      RUNTIME_FLEET_CATALOG_PROFILES_JSON_2: duplicate,
    })).toBeNull();
    expect(fleetCatalogProfilesFromEnv({
      RUNTIME_FLEET_CATALOG_PROFILES_JSON: duplicate,
      RUNTIME_FLEET_CATALOG_PROFILES_JSON_2: "not-json",
    })).toBeNull();
  });
  ...
  it("reports catalog and outbound flags truthfully when both private lanes are enabled", async () => {
    const agora = await encryptedCredentialRow("agora");
    const winerim = await encryptedCredentialRow("winerim");
    const fake = readinessDatabase({ agora: agora.row, winerim: winerim.row });
    const response = await createMiddlewareRuntimeExecutorWorker({ database: () => fake.adapter }).fetch(
    ...
    expect(body.enabledJobs).toContain("outbound.process");
  ...
  it("fences an exact rescue canary before the single-product Agora apply and DB persistence", async () => {
    ...
    const worker = createMiddlewareRuntimeExecutorWorker({
      ...
      catalogApply: () => remote,
      agoraMasterRefresh: successfulAgoraMasterRefresh(),
    });
  ...
  it("requires an operational master refresh before a bounded multi-product catalog apply", async () => {
    const agora = await encryptedCredentialRow("agora");
    const fake = readinessDatabase({ agora: agora.row });
    const remote = { applyAndReadback: vi.fn() } satisfies AgoraCatalogApplyAndReadbackPort;
    const worker = createMiddlewareRuntimeExecutorWorker({
    ...
    expect(response.status).toBe(503);
    await expect(response.json()).resolves.toMatchObject({
      failure: { message: "CATALOG_AGORA_MASTER_REFRESH_PORT_NOT_CONFIGURED" },
    });
    expect(remote.applyAndReadback).not.toHaveBeenCalled();
  });

  it("routes generic outbound through the scoped private executor when both flags are true", async () => {
    const fake = fakeDatabase();
    const response = await createMiddlewareRuntimeExecutorWorker({ database: () => fake.adapter }).fetch(
  ...
  it("rejects swapped active evidence with 403 before lease or provider calls", async () => {
    ...
    await expect(response.json()).resolves.toMatchObject({
      ok: false,
      failure: { httpStatus: 403, message: "WRITER_FENCE_ACTIVE_GRANT_MISMATCH" },
    });
