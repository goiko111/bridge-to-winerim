...
describe("Cloudflare runtime scheduler plan", () => {
  ...
  it("fans out only eligible connections and includes configured optional jobs", async () => {
    ...
    expect(messages.every(({ delaySeconds }) => delaySeconds >= 0 && delaySeconds < 245)).toBe(true);
    expect(messages.find(({ envelope }) => envelope.job === "catalog.sync-master")?.envelope.payload)
      .toEqual({ scheduled: true, formatTypes: ["BOTTLE", "GLASS", "MAGNUM"] });
  });

  it("spreads connections across the five-minute slot instead of creating a ten-second burst", () => {
    const delays = Array.from({ length: 32 }, (_, index) => (
      scheduledDelaySeconds(`connection-${index}`, "catalog.fetch-winerim")
    ));

    expect(new Set(delays).size).toBeGreaterThan(20);
    expect(Math.max(...delays) - Math.min(...delays)).toBeGreaterThan(120);
    expect(delays.every((delay) => delay >= 40 && delay < 220)).toBe(true);
  });
