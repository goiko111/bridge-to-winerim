...
import { BUSY_RETRY_DELAY_SECONDS } from "../../cloudflare/workers/middleware-runtime/src/queue";
import {
...
describe("staging-only Cloudflare middleware runtime Worker", () => {
  it("keeps the executor service timeout above chained upstream request timeouts", () => {
    ...
    expect(runtimeExecutorTimeoutMs("sales.auto-sync")).toBe(300_000);
    expect(runtimeExecutorTimeoutMs("sales.sync-intraday")).toBe(300_000);
    expect(runtimeExecutorTimeoutMs("outbound.process")).toBe(60_000);
    expect(runtimeIdempotencyLeaseMinutes("catalog.sync-master")).toBe(5);
    expect(runtimeIdempotencyLeaseMinutes("sales.auto-sync")).toBe(6);
    expect(runtimeIdempotencyLeaseMinutes("outbound.process")).toBe(2);
  });
  ...
  it("keeps a second message for the same connection and job out of the executor", async () => {
    const statements: string[] = [];
    const database = fakeDatabase((statement) => {
      statements.push(statement.text);
      if (statement.text.includes("AS active_count")) return result([{ active_count: 1 }]);
      return result();
    });
    const executor = { execute: vi.fn(async () => ({ ok: true as const })) };
    const body = await queueEnvelope("same-connection-job-busy");
    const message = { id: "cf-busy", attempts: 1, body, ack: vi.fn(), retry: vi.fn() };
    ...
    expect(message.retry).toHaveBeenCalledWith({ delaySeconds: BUSY_RETRY_DELAY_SECONDS });
    expect(message.ack).not.toHaveBeenCalled();
    expect(executor.execute).not.toHaveBeenCalled();
    expect(statements.some((text) => text.includes("pg_advisory_xact_lock"))).toBe(true);
    expect(statements.some((text) => text.includes("INSERT INTO public.runtime_idempotency"))).toBe(false);
  });
  ...
  it("persists retry disposition while preserving the same idempotency key", async () => {
    ...
    const executor = { execute: vi.fn(async () => ({
      ...
      failure: {
        httpStatus: 503,
        message: "AGORA_READBACK_HTTP_503",
        diagnostic: {
          operation: "agora.readback",
          route: "GET /api/export-master/?filter=Products",
          httpStatus: 503,
          elapsedMs: 1834,
          errorCode: "AGORA_READBACK_HTTP_503",
          bodySample: "upstream overloaded",
        },
      },
    })) };
    ...
    expect(statements.some((statement) => statement.text.includes("status = 'RETRY'"))).toBe(true);
    expect(statements.some((statement) => statement.text.includes("runtime_execution_log"))).toBe(true);
    expect(statements.some((statement) => statement.values.some((value) =>
      typeof value === "string"
      && value.includes("failureDiagnostic")
      && value.includes("/api/export-master/")
      && value.includes("upstream overloaded")
    ))).toBe(true);
  });
