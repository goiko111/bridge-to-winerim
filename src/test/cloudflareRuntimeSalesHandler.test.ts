...
async function acceptedImport(command: SalesImportCommand, stockApplied = true) {
  return {
    ok: true,
    lines: [{ lineId: command.lines[0]?.lineId, stockApplied }],
    evidence: {
      ...
      responseStatus: 200,
      certifiedOrderIds: [command.orderId],
      stockDisposition: command.stockDisposition,
    },
  };
}

async function acceptedStock(command: StockApplyCommand) {
  return {
    ok: true,
    stockMoved: true,
    evidence: {
      ...
      orderId: command.orderId,
      accepted: true as const,
      acceptedBy: "WINERIM_STOCK_READBACK" as const,
      reason: "fixture_stock_readback_certified",
      responseStatus: 200,
      certifiedOrderIds: [],
      stockDisposition: "APPLIED_EXACT_ONCE" as const,
    },
  };
}
...
function handlerPorts(
  ...
  return {
    ...
    applyStock: vi.fn(acceptedStock),
    importSales: vi.fn((command: SalesImportCommand) => acceptedImport(command)),
...
describe("Cloudflare runtime sales handler", () => {
  ...
  it("accepts live glass only with stockApplied or duplicate evidence", async () => {
    const rejectedPorts = handlerPorts({
      ...
      importSales: vi.fn((command: SalesImportCommand) => acceptedImport(command, false)),
    });
    ...
    const duplicatePorts = handlerPorts({
      ...
      importSales: vi.fn(async (command: SalesImportCommand) => ({
        ...await acceptedImport(command),
        duplicate: true,
        lines: [{ lineId: command.lines[0]?.lineId, duplicate: true }],
      })),
    });
  ...
  it("preserves a sanitized database driver code when a mutation query fails", async () => {
    const databaseError = Object.assign(new Error("DB_QUERY_FAILED"), { driverCode: "40001" });
    const ports = handlerPorts({
      winerimWineId: "47593",
      variant: "BOTTLE",
      stockId: "stock-1",
      ...
      applyStock: vi.fn().mockRejectedValue(databaseError),
    });
    ...
    expect(result.execution.items[0]).toMatchObject({
      status: "FAILED",
      retryable: true,
      error: "DB_QUERY_FAILED:40001",
    });
    expect(ports.releaseClaim).toHaveBeenCalledWith(expect.objectContaining({
      error: "DB_QUERY_FAILED:40001",
    }));
  });

  it("executes historical sales as non-live imports without applying stock", async () => {
    const importSales = vi.fn((command: SalesImportCommand) => acceptedImport(command, false));
    const ports = handlerPorts({
      winerimWineId: "47593",
      variant: "BOTTLE",
      stockId: "stock-1",
  ...
  it("applies only the atomic cumulative delta and keeps order identity stable", async () => {
    const applyStock = vi.fn(acceptedStock);
    const ports = handlerPorts({
      winerimWineId: "47593",
      variant: "BOTTLE",
      stockId: "stock-1",
  ...
  it("does not complete a history claim without Winerim acceptance evidence", async () => {
    const ports = handlerPorts({
      winerimWineId: "47593",
      variant: "BOTTLE",
      stockId: "stock-1",
    ...
    expect(ports.completeClaim).not.toHaveBeenCalled();
    expect(ports.releaseClaim).toHaveBeenCalledOnce();
  });

  it("contains a quarantined claim without mutating Winerim", async () => {
    const ports = handlerPorts({
      winerimWineId: "47593",
      variant: "BOTTLE",
      stockId: "stock-1",
      ...
      reserveClaim: vi.fn().mockResolvedValue({
        state: "QUARANTINED",
        appliedQuantity: 0,
        error: "SALES_CLAIM_TERMINAL_QUARANTINED",
      }),
    });
