...
describe("Agora provider-neutral read-only HTTP client", () => {
  ...
  it("includes protocol and query string in sanitized adapter diagnostics", async () => {
    const request = vi.fn(async () => {
      throw new Error("network down api-token=should-not-leak");
    });
    const client = createAgoraReadOnlyClient({
    ...
    try {
      await client.exportInvoices("2026-08-06");
    } catch (error) {
      observed = error;
    }

    expect(observed).toMatchObject<HttpAdapterError>({
      ...
      diagnostic: {
        ...
        method: "GET",
        protocol: "https:",
        host: "agora.example.test",
        path: "/api/export/?business-day=2026-08-06&filter=Invoices",
        url: "https://agora.example.test/api/export/?business-day=2026-08-06&filter=Invoices",
        bodySample: "Error: network down api-token=[REDACTED]",
      },
    });
    expect(JSON.stringify(observed)).not.toContain("agora-super-secret");
    expect(JSON.stringify(observed)).not.toContain("should-not-leak");
  });
