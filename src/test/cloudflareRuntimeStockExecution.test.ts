...
describe("Cloudflare runtime Winerim mutation execution", () => {
  ...
  it("accepts historical imported, duplicate and skipped lines without live or stock certification", async () => {
    for (const line of [
      { orderId: "history:glass:1", status: "imported", stockApplied: false },
      { orderId: "history:glass:1", status: "duplicate", duplicate: true },
      { orderId: "history:glass:1", status: "skipped", stockApplied: false },
    ]) {
  ...
  it("certifies the aggregate historical import contract only when the whole batch is accounted for", async () => {
    for (const body of [
      { imported: 1, skipped: 0, failed: 0, errors: [] },
      { imported: 0, skipped: 1, failed: 0, errors: [] },
    ]) {
      const transport = transportFor([{ status: 200, body }]);
      const result = await executeWinerimStockMutation({
        mode: "historical",
        orderId: "history:aggregate:1",
        soldAt: "2026-08-07T12:00:00Z",
        quantity: 1,
        soldStock: glass,
        stockSource: bottle,
        currentSourceStock: 99,
      ...
      expect(result).toMatchObject({
        ok: true,
        reason: "historical_aggregate_import_certified",
        certifiedOrderIds: ["history:aggregate:1"],
      });
    }

    for (const body of [
      { imported: 0, skipped: 0, failed: 0, errors: [] },
      { imported: 1, skipped: 0, failed: 1, errors: [{ index: 0 }] },
      { imported: 1, skipped: 0, failed: 0 },
    ]) {
      const transport = transportFor([{ status: 200, body }]);
      const result = await executeWinerimStockMutation({
        mode: "historical",
        orderId: "history:aggregate:unsafe",
        soldAt: "2026-08-07T12:00:00Z",
        quantity: 1,
        soldStock: glass,
        stockSource: bottle,
      ...
      expect(result).toMatchObject({
        ok: false,
        retryable: false,
        reason: "historical_line_not_certified_imported_or_duplicate",
      });
    }
  });
