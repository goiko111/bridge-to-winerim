...
describe("Cloudflare runtime catalog planning", () => {
  it("keeps deterministic Product.Id offsets for bottle, glass and magnum", async () => {
    const plan = await buildCatalogPlan(
      ...
      context({
        ...
        labelPolicy: { alwaysIncludeVintage: true },
      }),
    );
  ...
  it("preserves the Agora base sale format when updating an explicitly mapped legacy product", async () => {
    const plan = await buildCatalogPlan(request(), context({
      wines: [{
        winerimId: "156694",
        name: "Campillo Crianza",
        wineType: "tinto",
        variants: [{ format: "BOTTLE", salePrice: 27, explicitProductId: "818" }],
      }],
      existingProducts: [{
        productId: "818",
        baseSaleFormatId: "864",
        name: "CAMPILLO 2021 CRIANZA",
        familyId: "10",
        salePrice: 27,
      }],
      productIdPolicy: { explicitIds: { "156694:BOTTLE": "818" } },
    }));

    expect(plan.operations).toHaveLength(1);
    expect(plan.operations[0]).toMatchObject({
      kind: "update",
      desired: { productId: "818", baseSaleFormatId: "864" },
    });
  });

  it("does not invent a base sale format for a newly generated product", async () => {
    const plan = await buildCatalogPlan(request(), context({
      wines: [{
        winerimId: "156694",
        name: "Campillo Crianza",
        wineType: "tinto",
        variants: [{ format: "BOTTLE", salePrice: 27 }],
      }],
    }));

    expect(plan.operations[0]?.kind).toBe("create");
    expect(plan.operations[0]?.desired).not.toHaveProperty("baseSaleFormatId");
  });

  it("does not churn when a 20-character Agora ButtonText ends with whitespace", async () => {
    const name = "Muchada-Léclapart Univers";
    const desiredButtonText = `B ${name}`.slice(0, 20);
    const plan = await buildCatalogPlan(request(), context({
      wines: [{
        winerimId: "268254",
        name,
        wineType: "blanco",
        variants: [{ format: "BOTTLE", salePrice: 54 }],
      }],
      existingFamilies: [{ id: "904241", name: "BLANCOS WINERIM" }],
      existingProducts: [{
        productId: "768254",
        name: `B ${name}`,
        buttonText: desiredButtonText,
        familyId: "904241",
        salePrice: 54,
        costPrice: 0,
      }],
      familyRouting: {
        byWineType: { blanco: { id: "904241", name: "BLANCOS WINERIM" } },
      },
    }));

    expect(desiredButtonText.endsWith(" ")).toBe(true);
    expect(plan.operations).toHaveLength(1);
    expect(plan.operations[0]).toMatchObject({ kind: "unchanged", changedFields: [] });
  });

  it("does not churn when Agora collapses the long product name to ButtonText", async () => {
    const name = "Muchada-Léclapart Univers";
    const desiredButtonText = `B ${name}`.slice(0, 20);
    const plan = await buildCatalogPlan(request(), context({
      wines: [{
