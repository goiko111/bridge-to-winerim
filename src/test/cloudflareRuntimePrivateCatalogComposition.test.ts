...
function fixture(overrides: Partial<PrivateCatalogCompositionOptions> = {}) {
  ...
  const value: PrivateCatalogCompositionOptions = {
    ...
    credentials: { open },
    adapterFactory,
    agoraMasterRefresh: { refresh: refreshMaster },
...
describe("private catalog executor composition", () => {
  ...
  it("requires the separate apply switch and Agora credential before the atomic DB plan claim", async () => {
    ...
    expect(enabled.refreshMaster).toHaveBeenCalledOnce();
    expect(enabled.refreshMaster.mock.invocationCallOrder[0])
      .toBeLessThan(enabled.loadPlanningContext.mock.invocationCallOrder[0]);
  ...
  it("preserves sanitized fetch diagnostics for the queue retry contract", async () => {
    const configured = fixture({
      switches: { executionEnabled: true, fetchEnabled: true },
      refresh: {
        refresh: vi.fn(async () => ({
          ...
          diagnostic: {
            ...
            httpStatus: 429,
            elapsedMs: 12001,
            errorCode: "WINERIM_CATALOG_BULK_FAILED_OFFSET_700",
          },
        })),
      },
    });
    ...
    await expect(createPrivateCatalogLaneExecutor(configured.value).execute(
      ...
      failure: {
        httpStatus: 429,
        message: "CATALOG_FETCH_UNAVAILABLE",
        diagnostic: {
          operation: "winerim.catalog-bulk",
          route: "/api/v2/wines/bulk",
          httpStatus: 429,
