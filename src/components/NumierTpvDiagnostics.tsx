...
export default function NumierTpvDiagnostics({ activeTpvId, activeLocationId, diagnosing, diagnosisResult, onDiagnose }: Props) {
  ...
  const conclusionBadge = conclusion === "valid"
    ...
    : conclusion === "valid_no_sales_in_range"
    ? <Badge className="bg-blue-500/15 text-blue-700 dark:text-blue-400 border-blue-500/30">Valid · No sales in range</Badge>
    : conclusion === "range_too_large"
    ? <Badge className="bg-blue-500/15 text-blue-700 dark:text-blue-400 border-blue-500/30">Valid · Range too large</Badge>
    : conclusion === "suspicious"
