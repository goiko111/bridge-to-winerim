...
// Reuse the same interfaces from Agora since the data model is identical
...
export function useTcposConnection() {
  ...
  const saveConnection = async (data: {
    locationName: string;
    baseUrl: string;
    apiToken: string; // "user:password" format
  ...
  const testConnection = async (baseUrl: string, apiToken: string, winerimApiToken?: string) => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("tcpos-proxy", {
