...
runtime_canary_scope=$("${PSQL[@]}" -d "$DB_NAME" -Atc "SELECT (has_table_privilege('middleware_runtime', 'public.runtime_canary_connections', 'SELECT') AND NOT has_table_privilege('middleware_runtime', 'public.runtime_canary_connections', 'INSERT,UPDATE,DELETE') AND NOT has_table_privilege('middleware_runtime', 'public.stock_sync_log', 'SELECT,INSERT,UPDATE,DELETE') AND (SELECT count(*) = 0 FROM public.runtime_canary_connections))::int")
