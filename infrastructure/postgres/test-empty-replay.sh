...
if [ "$missing" -gt 0 ] || [ "$rls_missing" -gt 0 ] || [ "$security_definer_public" -gt 0 ] || [ "$public_policies" -gt 0 ] || [ "$runtime_lock_table" != "1" ] || [ "$runtime_lock_functions" != "1" ] || [ "$api_minimum_privileges" != "1" ] || [ "$legacy_lock_functions" != "0" ] || [ "$platform_helpers" -gt 0 ] || [ "$legacy_contact_columns" -gt 0 ] || [ "$secure_contact_columns" != "4" ]; then
  printf 'RESULT=EMPTY_REPLAY_FAILED missing=%s rls_missing=%s public_security_definer=%s public_policies=%s runtime_lock_table=%s runtime_lock_functions=%s api_minimum_privileges=%s legacy_lock_functions=%s platform_helpers=%s legacy_contact_columns=%s secure_contact_columns=%s\n' "$missing" "$rls_missing" "$security_definer_public" "$public_policies" "$runtime_lock_table" "$runtime_lock_functions" "$api_minimum_privileges" "$legacy_lock_functions" "$platform_helpers" "$legacy_contact_columns" "$secure_contact_columns" >&2
  exit 1
fi
