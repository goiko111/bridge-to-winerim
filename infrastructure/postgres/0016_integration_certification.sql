...
CREATE TABLE public.integration_monitoring_policies (
  ...
  offline_grace_minutes integer NOT NULL DEFAULT 30
    CHECK (offline_grace_minutes BETWEEN 0 AND 180),
  p0_after_minutes integer NOT NULL DEFAULT 20
    CHECK (p0_after_minutes BETWEEN 10 AND 240),
  healthy_cycles_required integer NOT NULL DEFAULT 2
    CHECK (healthy_cycles_required BETWEEN 2 AND 6),
  max_cycle_age_minutes integer NOT NULL DEFAULT 12
    CHECK (max_cycle_age_minutes BETWEEN 5 AND 30),
...
CREATE TABLE public.integration_certification_snapshots (
  ...
  state text NOT NULL CHECK (state IN (
    'ONLINE_OK', 'OFFLINE_EXPECTED', 'CATCHUP_PENDING', 'DEGRADED', 'P0'
  )),
  service_window_state text NOT NULL CHECK (service_window_state IN (
...
CREATE POLICY middleware_runtime_monitoring_policy_select
  ON public.integration_monitoring_policies
...
CREATE POLICY middleware_runtime_certification_snapshot_select
  ON public.integration_certification_snapshots
  FOR SELECT TO middleware_runtime
  USING (EXISTS (
    SELECT 1 FROM public.runtime_canary_connections scope
