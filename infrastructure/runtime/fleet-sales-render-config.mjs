...
export const FLEET_SALES_QUEUE = "winerim-rescue-prod-sales";
export const FLEET_SALES_DLQ = "winerim-rescue-prod-dead-letter";

export const FLEET_SALES_CONSUMER_BLOCK = `[[queues.consumers]]
queue = "${FLEET_SALES_QUEUE}"
max_batch_size = 1
max_batch_timeout = 5
max_retries = 3
max_concurrency = 1
dead_letter_queue = "${FLEET_SALES_DLQ}"`;
...
const defaultOutputPath = "/tmp/winerim-middleware-runtime-fleet-sales.toml";

export class FleetSalesRenderError extends Error {
  constructor(code) {
    super(code);
    this.name = "FleetSalesRenderError";
    this.code = code;
  }
}

function fail(code) {
  throw new FleetSalesRenderError(code);
}
...
export function renderFleetSalesRuntimeConfig({ baseSource, executorSource }) {
  if (typeof baseSource !== "string" || typeof executorSource !== "string") {
    fail("FLEET_SALES_BASE_CONFIG_REQUIRED");
  }
  validateFleetDeployment({ runtimeSource: baseSource, executorSource });
  if (!baseSource.includes(INERT_CONSUMER_MARKER)) {
    fail("FLEET_SALES_INERT_MARKER_MISSING");
  }
  if (baseSource.indexOf(INERT_CONSUMER_MARKER) !== baseSource.lastIndexOf(INERT_CONSUMER_MARKER)) {
    fail("FLEET_SALES_INERT_MARKER_NOT_UNIQUE");
  }

  const renderedSource = baseSource.replace(
    INERT_CONSUMER_MARKER,
    `# Sales-only activation. Keep catalog, stock, outbound and maintenance consumers absent.\n${FLEET_SALES_CONSUMER_BLOCK}`,
  );
  validateFleetSalesRuntimeConfig({ baseSource, executorSource, renderedSource });
  return renderedSource;
}

export function validateFleetSalesRuntimeConfig({ baseSource, executorSource, renderedSource }) {
  if (
    typeof baseSource !== "string"
    || typeof executorSource !== "string"
    || typeof renderedSource !== "string"
  ) {
    fail("FLEET_SALES_CONFIG_SOURCE_REQUIRED");
  }
  validateFleetDeployment({ runtimeSource: baseSource, executorSource });

  const blocks = consumerBlocks(renderedSource);
  if (blocks.length !== 1) fail("FLEET_SALES_CONSUMER_COUNT_MUST_BE_ONE");
  if (normalizeBlock(blocks[0]) !== normalizeBlock(FLEET_SALES_CONSUMER_BLOCK)) {
    fail("FLEET_SALES_CONSUMER_CONTRACT_MISMATCH");
  }
  ...
  if (expectedSource === baseSource) fail("FLEET_SALES_INERT_MARKER_MISSING");
  if (renderedSource !== expectedSource) fail("FLEET_SALES_BASE_CONFIG_CHANGED");
  ...
  return Object.freeze({
    ...
    maxRetries: 3,
    maxConcurrency: 1,
    deadLetterQueue: FLEET_SALES_DLQ,
  });
}
...
function parseCli(argv) {
  ...
  for (const argument of argv) {
    ...
    else fail("FLEET_SALES_RENDER_ARGUMENT_REJECTED");
  }
  return options;
}
...
function main() {
  ...
  const renderedSource = renderFleetSalesRuntimeConfig({ baseSource, executorSource });
  const result = validateFleetSalesRuntimeConfig({ baseSource, executorSource, renderedSource });
