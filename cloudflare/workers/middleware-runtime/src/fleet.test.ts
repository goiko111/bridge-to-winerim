...
describe("connection-scoped fleet producer", () => {
  ...
  it("always schedules every supported catalog format", async () => {
    const scopes = await resolveActiveFleetScheduledScopes(
      await scopeRows({ connectionId: CONNECTION_A, runId: "run-fleet-all-formats" }),
    );
    const messages = await buildFleetScheduledRuntimeMessages({
      cron: "*/5 * * * *",
      scheduledTimeMs: SCHEDULED_TIME,
    ...
    const sync = messages.find(({ envelope }) => envelope.job === "catalog.sync-master");
    expect(sync?.envelope.payload).toEqual({
      scheduled: true,
      formatTypes: ["BOTTLE", "GLASS", "MAGNUM"],
    });
  });

  it("schedules only the jobs explicitly allowed for a sales-only scope", async () => {
    const rows = await scopeRows({
      connectionId: CONNECTION_A,
      runId: "run-sales-only",
      intraday: true,
    });
    for (const row of rows) {
      row.runtime_fleet_job_allowlist = ["sales.auto-sync", "sales.sync-intraday"];
    }
    const scopes = await resolveActiveFleetScheduledScopes(rows);
    ...
    const sales = await buildFleetScheduledRuntimeMessages({
      cron: "*/5 * * * *",
      scheduledTimeMs: SCHEDULED_TIME,
      lane: "sales-stock",
      scopes,
    ...
    expect(sales.map(({ envelope }) => envelope.job)).toEqual([
      "sales.auto-sync",
      "sales.sync-intraday",
    ]);
  });

  it("rejects an active scope with no explicit runtime job allowlist", async () => {
    const rows = await scopeRows({ connectionId: CONNECTION_A, runId: "run-no-allowlist" });
    for (const row of rows) row.runtime_fleet_job_allowlist = null;
    ...
    expect(resolution.rejections).toEqual([{
      ...
      rowCount: 2,
      runIds: ["run-no-allowlist"],
    }]);
  });

  it("allows a one-minute catalog reconciliation only behind the explicit flag", async () => {
    const scopes = await resolveActiveFleetScheduledScopes(
      await scopeRows({ connectionId: CONNECTION_A, runId: "run-fleet-reconcile" }),
    );
    const blocked = await buildFleetScheduledRuntimeMessages({
      cron: "* * * * *",
      scheduledTimeMs: SCHEDULED_TIME,
      lane: "catalog",
      scopes,
    });
    const allowed = await buildFleetScheduledRuntimeMessages({
      cron: "* * * * *",
      scheduledTimeMs: SCHEDULED_TIME,
      lane: "catalog",
      scopes,
    ...
    const wrongLane = await buildFleetScheduledRuntimeMessages({
      cron: "* * * * *",
      scheduledTimeMs: SCHEDULED_TIME,
      lane: "sales-stock",
      scopes,
    ...
    expect(wrongLane).toEqual([]);
    expect(allowed).toHaveLength(2);
    expect(allowed.map(({ envelope }) => envelope.job)).toEqual([
      "catalog.fetch-winerim",
      "catalog.sync-master",
    ]);
    expect(allowed.every(({ envelope }) => (
      envelope.source.scheduledSlot === "2026-08-04T12:00:00.000Z"
    ))).toBe(true);
  });
  ...
  it("loads only ACTIVE scopes and needs only the configured lane binding", async () => {
    ...
    expect(batch.map(({ body }) => body.job)).toEqual([
      "catalog.fetch-winerim",
      "catalog.sync-master",
      "catalog.fetch-winerim",
      "catalog.sync-master",
  ...
  it("can schedule catalog and outbound lanes from one fleet cron worker", async () => {
    const rows = [
      ...await scopeRows({ connectionId: CONNECTION_A, runId: "run-fleet-a" }),
      ...await scopeRows({ connectionId: CONNECTION_B, runId: "run-fleet-b" }),
    ];
    const database = fakeDatabase(() => result(rows as Record<string, unknown>[]));
    const catalog = {
      sendBatch: vi.fn(async (_messages: RuntimeQueueSendMessage[]) => undefined),
    } satisfies RuntimeQueueProducer;
    const outbound = {
    ...
    const dispatched = await runRuntimeScheduled(
      { cron: "*/5 * * * *", scheduledTime: SCHEDULED_TIME },
      {
        ...
        RUNTIME_MODE: "fleet-producer",
        FLEET_RUNTIME_LANE: "catalog",
        FLEET_RUNTIME_SCHEDULED_LANES: "catalog,outbound-queue",
        RUNTIME_EXECUTION_ENABLED: "true",
        MIDDLEWARE_DB: { connectionString: "postgres://runtime.invalid/rescue" },
        RUNTIME_EXECUTOR: { fetch: vi.fn() },
        MIDDLEWARE_CATALOG_QUEUE: catalog,
        MIDDLEWARE_OUTBOUND_QUEUE: outbound,
      },
      dependencies,
    );

    expect(dispatched).toEqual({ status: "dispatched", connections: 2, messages: 6 });
    expect(catalog.sendBatch).toHaveBeenCalledOnce();
    expect(outbound.sendBatch).toHaveBeenCalledOnce();
    expect(vi.mocked(catalog.sendBatch).mock.calls[0][0].map(({ body }) => body.job))
      .toEqual([
        "catalog.fetch-winerim",
        "catalog.sync-master",
        "catalog.fetch-winerim",
    ...
    expect(vi.mocked(outbound.sendBatch).mock.calls[0][0].map(({ body }) => body.lane))
      .toEqual(["outbound-queue", "outbound-queue"]);
  });
  ...
  it("uses valid SQL when the staging scheduler loads unscoped connections", async () => {
    const database = fakeDatabase((statement) => {
      expect(statement.text).toContain("FROM public.pos_connections");
      expect(statement.text).not.toMatch(/open_tickets_sync_enabled,\s+FROM/);
      return result([{
    ...
    const dispatched = await runRuntimeScheduled(
      { cron: "*/5 * * * *", scheduledTime: SCHEDULED_TIME },
      {
        ...
        RUNTIME_EXECUTOR: { fetch: vi.fn() },
        MIDDLEWARE_CATALOG_QUEUE: queue,
        MIDDLEWARE_SALES_STOCK_QUEUE: queue,
        MIDDLEWARE_SALES_IMPORT_QUEUE: queue,
        MIDDLEWARE_STOCK_SYNC_QUEUE: queue,
        MIDDLEWARE_OUTBOUND_QUEUE: queue,
        MIDDLEWARE_MAINTENANCE_QUEUE: queue,
      },
      dependencies,
    );

    expect(dispatched.status).toBe("dispatched");
    expect(dispatched.connections).toBe(1);
  });
});
