...
export type ExactSalesMapping = SalesLineResolution & {
  ...
  providerSaleFormatId?: string;
  providerProductName: string;
  providerSaleFormatName?: string;
  mappingId: string;
  mappingStatus: "CONFIRMED";
};

export type ExactSalesLineIdentity = {
  providerProductId: string;
  saleFormatId?: string;
};
...
export type PostgresSalesAdapter = SalesPlanningPorts &
  ...
  > & {
    ...
    readExactMappingsForLines(lines: ExactSalesLineIdentity[]): Promise<ExactSalesMapping[]>;
...
export type PostgresSalesAdapterOptions = {
  ...
  requireNativeSaleFormatIdentity?: boolean;
};
