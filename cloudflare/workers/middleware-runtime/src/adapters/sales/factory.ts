  ...
  const prepare = async (input: PrepareSalesRunInput): Promise<PreparedSalesRun> => {
    ...
    const adapter = (dependencies.adapterFactory ?? ((factoryInput) => createPostgresSalesAdapter(
      factoryInput.database,
      {
        ...
        requireNativeSaleFormatIdentity: configFlag(
          factoryInput.connection,
          "agora_native_sale_format_identity_v1",
        ),
      },
    )))({ database: dependencies.database, connection });
    const requestedProviderProductIds = providerProductIds(documents);
    const exactMappings = await adapter.readExactMappingsForLines(
      documents.flatMap((document) => document.lines),
    );
    const mappingIndex = exactSalesMappingIndex(exactMappings);
    ...
    const handler = await handleSalesRun({
      ...
      resolveLine: async ({ line }) => exactSalesMappingForLine(mappingIndex, line, {
        requireNativePair: configFlag(connection, "agora_native_sale_format_identity_v1"),
      }),
