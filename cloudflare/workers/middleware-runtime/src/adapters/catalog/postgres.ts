...
function catalogApplyDiagnosticCode(error: unknown): string {
  if (error instanceof PostgresCatalogAdapterInvariantError) return error.code;
  if (error instanceof DatabaseAdapterError) {
    return [error.code, error.driverCode].filter(Boolean).join(":");
  }
  return error instanceof Error ? error.name : "CATALOG_DB_UNKNOWN_ERROR";
}
...
function productFromRecord(row: Record<string, unknown>): CatalogExistingProduct | null {
  ...
  const rawPayload = record(field(row, "raw_payload", "rawPayload"));
  const rawAttributes = record(field(rawPayload, "attributes", "Attributes"));
  const baseSaleFormatId = nullableText(
    field(row, "BaseSaleFormatId", "baseSaleFormatId", "base_sale_format_id")
      ?? field(rawAttributes, "BaseSaleFormatId", "baseSaleFormatId", "base_sale_format_id")
      ?? field(rawPayload, "BaseSaleFormatId", "baseSaleFormatId", "base_sale_format_id"),
  );
  ...
  const rawSalePrice = field(row, "SalePrice", "salePrice", "MainPrice", "mainPrice", "Price", "price");
...
function explicitProductIds(
  ...
  for (const row of tracking) {
    ...
    const verifiedWinerimProduct = text(row.sync_status).toUpperCase() === "VERIFIED"
      && text(row.source).toUpperCase() === "WINERIM";
    if (!key || rejected.has(key) || !verifiedWinerimProduct || !/^\d+$/.test(productId)) continue;
  ...
  for (const row of mappings) {
    ...
    if (
      ...
      || explicit[key]
      || !/^\d+$/.test(productId)
      || !EXPLICIT_MAPPING_STATUSES.has(text(row.status).toUpperCase())
    ) continue;
    explicit[key] = productId;
  }
  for (const row of tracking) {
...
function mergeExistingProducts(
  ...
  mappings: readonly MappingRow[],
  tracking: readonly TrackingRow[],
  masterObservedAt: unknown,
): CatalogExistingProduct[] {
  const products = new Map<string, CatalogExistingProduct>();
  const masterTimestamp = Date.parse(text(masterObservedAt));
  const masterObservedTimestamp = Number.isFinite(masterTimestamp) ? masterTimestamp : 0;
  ...
  const merge = (product: CatalogExistingProduct | null, authoritative = false): void => {
    ...
    products.set(product.productId, {
      ...
      baseSaleFormatId: current.baseSaleFormatId ?? product.baseSaleFormatId,
  ...
  for (const row of providerProducts) {
    const raw = record(row.raw_payload);
    const providerTimestamp = Date.parse(text(row.updated_at));
    const providerIsNewer = Number.isFinite(providerTimestamp)
      && providerTimestamp >= masterObservedTimestamp;
  ...
  try {
    ...
    const context: CatalogPlanningContext = {
      ...
      existingProducts: mergeExistingProducts(
        ...
        mappingResult.rows,
        trackingResult.rows,
        master.fetched_at || master.updated_at,
      ),
...
function planResult(plan: CatalogPlan): Record<string, unknown> {
  return {
    ...
    operations: plan.operations.map((operation) => ({
      ...
      desiredName: operation.desired.label.name,
      desiredButtonText: operation.desired.label.buttonText,
    })),
  ...
  const result = await transaction.query<Record<string, unknown>>(sql`
    ...
    SELECT
      ...
      CASE
        WHEN EXISTS (
          SELECT 1
          FROM jsonb_array_elements(COALESCE(ww.raw_payload -> 'prices', '[]'::jsonb)) AS variant_price(value)
          WHERE CASE
            WHEN lower(COALESCE(variant_price.value ->> 'variant', '')) IN ('botella', 'bottle') THEN 'BOTTLE'
            WHEN lower(COALESCE(variant_price.value ->> 'variant', '')) IN ('copa', 'glass') THEN 'GLASS'
            WHEN lower(COALESCE(variant_price.value ->> 'variant', '')) IN ('magnum', 'mag') THEN 'MAGNUM'
            ELSE ''
          END = ${desired.format}
            AND lower(COALESCE(variant_price.value -> 'erpStock' ->> 'stockActive', 'false')) = 'true'
        ) THEN 'RESCUE_EXACT_ID_WINE_VARIANT'
...
async function persistVerifiedProviderShadow(
  transaction: DatabaseTransaction,
  connectionId: string,
  operation: CatalogProductOperation,
): Promise<void> {
  const desired = operation.desired;
  const product = {
    Id: desired.productId,
    Name: desired.label.name,
    ButtonText: desired.label.buttonText,
    FamilyId: desired.family.id,
    SalePrice: desired.salePrice,
    CostPrice: desired.costPrice,
    UseAsDirectSale: desired.useAsDirectSale,
    SaleableAsMain: desired.saleableAsMain,
    provider_product_id: desired.productId,
    name: desired.label.name,
    buttonText: desired.label.buttonText,
    family_id: desired.family.id,
    family: desired.family.name,
    price: desired.salePrice,
    sale_format: desired.format,
    winerim_wine_id: desired.winerimId,
  };

  const provider = await transaction.query<Record<string, unknown>>(sql`
    ...
    ON CONFLICT (connection_id, provider_product_id) DO UPDATE SET
      ...
      sale_format = EXCLUDED.sale_format,
      price = EXCLUDED.price,
      is_wine_candidate = true,
      raw_payload = COALESCE(provider_products.raw_payload, '{}'::jsonb) || EXCLUDED.raw_payload,
      sync_status = 'SYNCED',
      sync_error = NULL,
      last_synced_at = now(),
      winerim_wine_id = EXCLUDED.winerim_wine_id,
  ...
  if (provider.rowCount !== 1) {
    throw new PostgresCatalogAdapterInvariantError("CATALOG_DB_PROVIDER_SHADOW_NOT_PERSISTED");
  }

  const master = await transaction.query<Record<string, unknown>>(sql`
    UPDATE public.agora_master_data
    SET products_summary_json = CASE
      WHEN EXISTS (
        SELECT 1
        FROM jsonb_array_elements(COALESCE(products_summary_json, '[]'::jsonb)) AS current_product(value)
        WHERE COALESCE(
          current_product.value ->> 'Id',
          current_product.value ->> 'id',
          current_product.value ->> 'ProductId',
          current_product.value ->> 'productId',
          current_product.value ->> 'provider_product_id'
        ) = ${desired.productId}
      ) THEN (
        SELECT COALESCE(
          jsonb_agg(
            CASE
              ...
              THEN current_product.value || ${JSON.stringify(product)}::jsonb
              ELSE current_product.value
            END
            ORDER BY current_product.ordinality
          ),
          '[]'::jsonb
        )
        FROM jsonb_array_elements(COALESCE(products_summary_json, '[]'::jsonb))
          WITH ORDINALITY AS current_product(value, ordinality)
      )
      ELSE COALESCE(products_summary_json, '[]'::jsonb) || jsonb_build_array(${JSON.stringify(product)}::jsonb)
    END,
    updated_at = now()
    WHERE connection_id = ${connectionId}::uuid
  ...
  if (master.rowCount !== 1) {
    throw new PostgresCatalogAdapterInvariantError("CATALOG_DB_MASTER_SHADOW_NOT_PERSISTED");
  }
}
...
async function applyPlan(
  ...
  try {
    ...
    if (error instanceof PostgresCatalogAdapterInvariantError) {
      return { ok: false, code: "APPLY_REJECTED", diagnosticCode: catalogApplyDiagnosticCode(error) };
    }
    return { ok: false, code: "APPLY_UNAVAILABLE", diagnosticCode: catalogApplyDiagnosticCode(error) };
  }
  ...
  try {
    return await database.transaction(async (transaction) => {
      ...
      if (claim === "DUPLICATE") {
        for (const operation of input.plan.operations) {
          await persistVerifiedProviderShadow(transaction, input.plan.connectionId, operation);
        }
    ...
    if (error instanceof PostgresCatalogAdapterInvariantError) {
      ...
      return {
        ok: false,
        code: conflict ? "APPLY_CONFLICT" : "APPLY_REJECTED",
        diagnosticCode: catalogApplyDiagnosticCode(error),
      };
    }
    return { ok: false, code: "APPLY_UNAVAILABLE", diagnosticCode: catalogApplyDiagnosticCode(error) };
  }
