...
function labelsFor(
  ...
  const alwaysIncludeVintage = policy.alwaysIncludeVintage === true;
...
function changedFields(
  ...
  const existingName = normalizedText(existing.name);
  const existingButtonText = normalizedText(existing.buttonText);
  const desiredName = normalizedText(desired.label.name);
  const desiredButtonText = normalizedText(desired.label.buttonText);
  const providerCollapsedNameToButton = !!existingButtonText
    && existingName === existingButtonText
    && existingButtonText === desiredButtonText;
  if (existingName !== desiredName && !providerCollapsedNameToButton) changed.push("name");
  if (existing.buttonText !== undefined && existing.buttonText !== null
      && existingButtonText !== desiredButtonText) changed.push("buttonText");
...
export async function buildCatalogPlan(
  ...
  for (const candidate of candidates) {
    ...
    const desired: CatalogProductOperation["desired"] = {
      ...
      ...(existing?.baseSaleFormatId ? { baseSaleFormatId: existing.baseSaleFormatId } : {}),
