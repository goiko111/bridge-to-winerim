...
const IMPORT_SUCCESS_STATUSES = new Set(["imported", "duplicate", "skipped"]);
...
function aggregateHistoricalImportCertified(body: unknown, requestedCount: number): boolean {
  const payload = asRecord(body);
  const imported = Number(payload.imported);
  const skipped = Number(payload.skipped);
  const failed = Number(payload.failed);
  const errors = payload.errors;
  if (![imported, skipped, failed].every(Number.isInteger)) return false;
  if (imported < 0 || skipped < 0 || failed !== 0) return false;
  if (!Array.isArray(errors) || errors.length !== 0) return false;
  return imported + skipped === requestedCount;
}
...
function salesImportDecision(
  ...
  if (
    !plan.requiresLiveStockCertification
    && linesByOrderId.size === 0
    && aggregateHistoricalImportCertified(response.body, requestedIds.length)
  ) {
    return {
      action: "success",
      certifiedOrderIds: requestedIds,
      retryableOrderIds: [],
      terminalOrderIds: [],
