  ...
  for (const group of groups.values()) {
    ...
    const action = mode === "HISTORICAL" || !resolution.stockActive || resolution.variant === "GLASS"
      ? {
        ...
        stockDisposition: mode === "HISTORICAL"
          ? "HISTORY_ONLY_NO_STOCK" as const
          : resolution.stockActive && resolution.variant === "GLASS"
            ? "APPLIED_EXACT_ONCE" as const
            : "SALES_ONLY_NO_STOCK" as const,
        lines: [importLine],
      }
      : {
