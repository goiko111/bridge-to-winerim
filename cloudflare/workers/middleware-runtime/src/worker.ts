...
import type { RuntimeFailure } from "./retry";
...
export interface MiddlewareRuntimeEnv extends WriterFenceClientEnvironment {
  ...
  FLEET_RUNTIME_SCHEDULED_LANES?: string;
  FLEET_RUNTIME_ALLOW_ONE_MINUTE_RECONCILIATION?: string;
...
interface RuntimeActiveJobRow extends Record<string, unknown> {
  active_count: number;
}
...
function runtimeFailureDiagnostic(failure: RuntimeFailure): Record<string, unknown> {
  return {
    ...(typeof failure.httpStatus === "number" ? { failureHttpStatus: failure.httpStatus } : {}),
    ...(typeof failure.message === "string" ? { failureMessage: failure.message } : {}),
    ...(failure.diagnostic ? { failureDiagnostic: failure.diagnostic } : {}),
  };
}
...
function defaultExecutor(env: MiddlewareRuntimeEnv): RuntimeExecutor | null {
  ...
  return {
    async execute(envelope): Promise<RuntimeExecutionResult> {
      ...
      const executorTimeoutMs = runtimeExecutorTimeoutMs(envelope.job);
      ...
      try {
        ...
        return {
          ok: false,
          failure: {
            httpStatus: timedOut ? 408 : 503,
            message: timedOut ? "RUNTIME_EXECUTOR_TIMEOUT" : "RUNTIME_EXECUTOR_UNAVAILABLE",
            diagnostic: {
              operation: "runtime.executor",
              route: "service-binding:/v1/execute",
              elapsedMs: Date.now() - startedAt,
              errorCode: timedOut ? "RUNTIME_EXECUTOR_TIMEOUT" : "RUNTIME_EXECUTOR_UNAVAILABLE",
            },
          },
        };
      } finally {
      ...
      return {
        ...
        failure: {
          ...
          diagnostic: record(payload.failure?.diagnostic),
        },
      };
    },
  };
...
function missingBindingNames(env: MiddlewareRuntimeEnv, executor?: RuntimeExecutor | null): string[] {
  ...
  const scheduledFleetLanes = isFleetProducer(env)
    ? parseFleetScheduledLanes(env.FLEET_RUNTIME_SCHEDULED_LANES ?? env.FLEET_RUNTIME_LANE)
    : [];
  const missing = isCanaryConsumer(env)
    ...
    : isFleetProducer(env)
      ? [
          ...
          scheduledFleetLanes.length === 0 ? "FLEET_RUNTIME_SCHEDULED_LANES" : null,
          ...scheduledFleetLanes.map((lane) => (
            !queueForLane(env, lane) ? `FLEET_QUEUE_${lane.toUpperCase()}` : null
          )),
...
export function createPersistentRuntimeQueueHooks(
  ...
  return {
    async reserve(envelope) {
      ...
      const leaseMinutes = runtimeIdempotencyLeaseMinutes(envelope.job);
      return database.transaction(async (transaction) => {
        await transaction.query(sql`
          SELECT pg_advisory_xact_lock(
            hashtextextended(${runtimeSingleFlightKey(envelope)}, 0)
          )
        `);
        const activeJob = await transaction.query<RuntimeActiveJobRow>(sql`
          SELECT count(*)::int AS active_count
          FROM public.runtime_idempotency
          WHERE connection_id = ${envelope.connectionId}::uuid
            AND job = ${envelope.job}
            AND idempotency_key <> ${envelope.idempotencyKey}
            AND status = 'RUNNING'
            AND lease_expires_at > now()
        `);
        if (Number(activeJob.rows[0]?.active_count ?? 0) > 0) {
        ...
        const inserted = await transaction.query<RuntimeAttemptRow>(sql`
          INSERT INTO public.runtime_idempotency (
            ...
            now() + (${leaseMinutes} * interval '1 minute'),
        ...
        const released = await transaction.query<RuntimeAttemptRow>(sql`
          ...
          SET status = 'RETRY', lease_expires_at = NULL,
            result = ${JSON.stringify({
              ...
              state: "retry",
              ...runtimeFailureDiagnostic(disposition.failure),
            })}::jsonb,
            updated_at = now()
          WHERE idempotency_key = ${envelope.idempotencyKey}
            AND message_id = ${envelope.messageId}
        ...
        const released = await transaction.query<RuntimeAttemptRow>(sql`
          ...
          SET status = 'RETRY', lease_expires_at = NULL,
            result = ${JSON.stringify({
              ...
              ...runtimeFailureDiagnostic(input.disposition.failure),
            })}::jsonb,
            updated_at = now()
          WHERE idempotency_key = ${envelope.idempotencyKey}
            AND message_id = ${envelope.messageId}
    ...
    async recordTerminal(envelope, input) {
      ...
      await database.transaction(async (transaction) => {
        ...
        await insertExecutionLog(transaction, envelope, "TERMINAL", row.attempt, {
          ...
          detail: {
            reason: input.reason,
            ...(input.disposition ? runtimeFailureDiagnostic(input.disposition.failure) : {}),
          },
        });
      }, { isolationLevel: "serializable" });
      leases.delete(envelope);
...
async function loadFleetScheduledConnections(database: DatabaseAdapter) {
  const result = await database.query<FleetScopeDatabaseRow>(sql`
    SELECT
      ...
      connection.provider_config->'runtime_fleet_job_allowlist'
        AS runtime_fleet_job_allowlist,
...
export async function runRuntimeScheduled(
  ...
  if (fleetLane) {
    ...
    messages = (await Promise.all(scheduledFleetLanes.map((lane) => (
      buildFleetScheduledRuntimeMessages({
          cron: controller.cron,
          scheduledTimeMs: controller.scheduledTime,
          lane,
          scopes,
          allowOneMinuteReconciliation:
            env.FLEET_RUNTIME_ALLOW_ONE_MINUTE_RECONCILIATION === "true",
        })
    )))).flat();
  } else {
