...
export const FLEET_FULL_LANES_RUNTIME_JOB_ALLOWLIST = Object.freeze([
  "sales.auto-sync",
  "sales.sync-intraday",
...
export type FleetFullLanesRuntimeJob = typeof FLEET_FULL_LANES_RUNTIME_JOB_ALLOWLIST[number];
export type FleetRuntimeJob = FleetSalesRuntimeJob | FleetFullLanesRuntimeJob;
...
export type ActiveFleetScope = Readonly<{
  ...
  runtimePolicyProfile: "sales-only-v1" | "full-lanes-v1";
...
function exactFleetFullLanesPolicy(row: ActiveFleetScopeRow): readonly FleetFullLanesRuntimeJob[] | null {
  if (
  ...
  const providerConfig = row.provider_config as Record<string, unknown>;
  const fleetAllowlist = providerConfig.runtime_fleet_job_allowlist;
  const salesAllowlist = providerConfig.runtime_sales_job_allowlist;
  if (
    providerConfig.runtime_fleet_profile !== "full-lanes-v1"
    || !Array.isArray(fleetAllowlist)
    || fleetAllowlist.length !== FLEET_FULL_LANES_RUNTIME_JOB_ALLOWLIST.length
    || fleetAllowlist.some((job, index) => job !== FLEET_FULL_LANES_RUNTIME_JOB_ALLOWLIST[index])
    || !Array.isArray(salesAllowlist)
    || salesAllowlist.length !== FLEET_SALES_RUNTIME_JOB_ALLOWLIST.length
    || salesAllowlist.some((job, index) => job !== FLEET_SALES_RUNTIME_JOB_ALLOWLIST[index])
    || typeof providerConfig.runtime_sales_cutover_business_day !== "string"
    || !/^\d{4}-\d{2}-\d{2}$/.test(providerConfig.runtime_sales_cutover_business_day)
    || providerConfig.intraday_sales_sync_enabled !== true
    || providerConfig.open_tickets_sync_enabled !== false
    || providerConfig.open_tickets_stock_sync_enabled !== false
  ...
  return FLEET_FULL_LANES_RUNTIME_JOB_ALLOWLIST;
}
...
function validActiveFleetScope(row: ActiveFleetScopeRow): ActiveFleetScope | null {
  ...
  const runtimeFullLanesJobAllowlist = exactFleetFullLanesPolicy(row);
  const runtimeSalesJobAllowlist = runtimeFullLanesJobAllowlist
    ? FLEET_SALES_RUNTIME_JOB_ALLOWLIST
    : exactFleetSalesPolicy(row);
  const runtimeJobAllowlist = runtimeFullLanesJobAllowlist ?? runtimeSalesJobAllowlist;
  if (
  ...
  return Object.freeze({
    ...
    runtimePolicyProfile: runtimeFullLanesJobAllowlist ? "full-lanes-v1" : "sales-only-v1",
    runtimeJobAllowlist,
    runtimeSalesJobAllowlist,
  });
}
