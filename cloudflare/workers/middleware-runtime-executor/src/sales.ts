...
} from "../../middleware-runtime/src/adapters/sales";
import type {
  ExactSalesMapping,
...
import {
  ...
  type SalesMutationAcceptanceEvidence,
  type SalesPlan,
  type SalesRunKind,
  type SalesVariant,
...
export const DEFAULT_MAX_SALES_INTENTS_PER_RUN = 4;
const DEFAULT_AGORA_READ_TIMEOUT_MS = 10_000;
...
export function boundSalesExecutionPlan(
  plan: SalesPlan,
  maxIntents = DEFAULT_MAX_SALES_INTENTS_PER_RUN,
): { plan: SalesPlan; pendingIntentCount: number } {
  if (!Number.isInteger(maxIntents) || maxIntents < 1 || maxIntents > 20) {
    throw new SalesLaneError("SALES_MAX_INTENTS_INVALID", 422, false);
  }
  const intents = plan.intents.slice(0, maxIntents);
  return {
    plan: { ...plan, intents },
    pendingIntentCount: Math.max(0, plan.intents.length - intents.length),
  };
}
...
export class SalesLaneError extends Error {
  constructor(
    ...
    readonly diagnostic?: RuntimeFailureDiagnosticInput,
  ) {
    super(code);
...
function nativeSaleFormatIdentityRequired(connection: SalesLaneConnection): boolean {
  return boolean(connection.providerConfig.agora_native_sale_format_identity_v1);
}

function diagnosticBodySample(value: unknown): string | undefined {
  const raw = typeof value === "string" ? value : JSON.stringify(value ?? null);
  if (typeof raw !== "string") return undefined;
  const normalized = raw
    .replace(/(api[-_ ]?token|authorization|bearer|password|secret|credential)(\s*[:=]\s*)[^\s,;]+/gi, "$1$2[REDACTED]")
    .replace(/\s+/g, " ")
    .trim();
  return normalized ? normalized.slice(0, 256) : undefined;
}

function unknownSalesDiagnostic(error: unknown): RuntimeFailureDiagnosticInput {
  if (error instanceof HttpAdapterError) {
    const method = error.diagnostic.method ?? "GET";
    const routeTarget = typeof error.diagnostic.url === "string" && error.diagnostic.url
      ? error.diagnostic.url
      : [
        error.diagnostic.protocol,
        error.diagnostic.host,
        error.diagnostic.path,
      ].filter(Boolean).join(" ");
    const route = [method, routeTarget].filter(Boolean).join(" ");
    return {
      operation: error.diagnostic.operation ?? "sales.execute",
      route,
      httpStatus: error.diagnostic.status,
      elapsedMs: error.diagnostic.durationMs,
      errorCode: error.code,
      bodySample: diagnosticBodySample(error.diagnostic.bodySample ?? `${error.name}: ${error.code}`),
    };
  }
  ...
  return {
    ...
    bodySample: diagnosticBodySample(`${name}: ${message}`),
  };
}
...
function parseXmlAttributes(value: string): JsonRecord {
...
function parseXmlScalarChildren(value: string): JsonRecord {
  const result: JsonRecord = {};
  const childPattern = /<([A-Za-z_][\w:.-]*)\b[^>]*>([^<]*)<\/\1>/g;
  let match: RegExpExecArray | null;
  while ((match = childPattern.exec(value))) {
    const key = match[1];
...
function parseAgoraInvoiceXml(payload: string): ParsedInvoices {
  const xml = payload.trim();
  if (!xml.startsWith("<")) return { recognized: false, invoices: [] };
  // Agora's export endpoint wraps invoices in <Export>. On a business day
  // without invoices it returns an empty <Export .../> document, which is a
  // valid zero-result response rather than an unknown payload.
  if (!/<(?:Export|Invoices|InvoiceModel|Invoice)(?:\s|\/?>)/i.test(xml)) {
    return { recognized: false, invoices: [] };
  }

  const invoices: JsonRecord[] = [];
  const invoicePattern = /<(InvoiceModel|Invoice)\b([^>]*?)(?:\/>|>([\s\S]*?)<\/\1>)/gi;
  let invoiceMatch: RegExpExecArray | null;
  while ((invoiceMatch = invoicePattern.exec(xml))) {
    const body = invoiceMatch[3] ?? "";
    const invoice: JsonRecord = {
      ...parseXmlScalarChildren(body),
      ...parseXmlAttributes(invoiceMatch[2] ?? ""),
    };
    const lines: JsonRecord[] = [];
    const linePattern = /<(InvoiceLineModel|InvoiceLine|Line)\b([^>]*?)(?:\/>|>([\s\S]*?)<\/\1>)/gi;
    let lineMatch: RegExpExecArray | null;
    while ((lineMatch = linePattern.exec(body))) {
      lines.push({
        ...parseXmlScalarChildren(lineMatch[3] ?? ""),
        ...parseXmlAttributes(lineMatch[2] ?? ""),
      });
    }
    invoice.Lines = lines;
    invoices.push(invoice);
  }
  return { recognized: true, invoices };
}

export function parseAgoraInvoicesPayload(payload: unknown): ParsedInvoices {
  // Agora returns either `{}` or an empty 200 response when a business day has
  // no invoices, depending on the reverse proxy in front of the TPV.
  if (payload === null || payload === undefined) return { recognized: true, invoices: [] };
  if (typeof payload === "string") {
    const trimmed = payload.trim();
    if (!trimmed) return { recognized: true, invoices: [] };
    if (trimmed.startsWith("<")) return parseAgoraInvoiceXml(trimmed);
    try {
      return parseAgoraInvoicesPayload(JSON.parse(trimmed));
    } catch {
      return { recognized: false, invoices: [] };
    }
  }
  ...
  if (Object.keys(root).length === 0) return { recognized: true, invoices: [] };
...
async function loadDefinitiveDocuments(
  ...
  const client = createAgoraReadOnlyClient({
    ...
    timeoutMs: dependencies.agoraReadTimeoutMs ?? DEFAULT_AGORA_READ_TIMEOUT_MS,
  });
  const response = await client.exportInvoices(businessDay);
  if (!response.ok) {
    throw new SalesLaneError(`AGORA_INVOICES_HTTP_${response.status}`, 503, true, {
      operation: "agora.export.invoices",
      route: `GET /api/export/?business-day=${businessDay}&filter=Invoices`,
      httpStatus: response.status,
      errorCode: `AGORA_INVOICES_HTTP_${response.status}`,
      bodySample: diagnosticBodySample(response.body),
    });
  }
...
async function loadOpenTicketDocuments(
  ...
  if (!response.ok) {
    throw new SalesLaneError(`AGORA_OPEN_TICKETS_HTTP_${response.status}`, 503, true, {
      operation: "agora.export.open-tickets",
      route: "GET /api/export/tickets/",
      httpStatus: response.status,
      errorCode: `AGORA_OPEN_TICKETS_HTTP_${response.status}`,
      bodySample: diagnosticBodySample(response.body),
    });
  }
...
function normalizeMappedLine(
  ...
  exactMappings: ReadonlyMap<string, ExactSalesMapping>,
): ProviderSalesLine {
  // Preserve both native Agora identifiers. Rewriting ProductId to
  // SaleFormatId destroys the composite identity and can collide with an
  // unrelated product that happens to use the same numeric ID.
  void exactMappings;
  return line;
}
...
function classifyLine(
  ...
  requireNativePair: boolean,
): SalesLineClassification {
  const providerProductId = text(line.providerProductId);
  const saleFormatId = text(line.saleFormatId);
  if (
    (providerProductId === "" || providerProductId === "0")
    && (saleFormatId === "" || saleFormatId === "0")
    && !line.suggestedVariant
    && !familyKey(line.familyName)
  ) {
    return "NOT_WINE";
  }
  if (exactSalesMappingForLine(exactMappings, line, { requireNativePair })) return "WINE";
...
function mapStockRun(
  ...
  const evidence = (stockDisposition: SalesMutationAcceptanceEvidence["stockDisposition"]): SalesMutationAcceptanceEvidence => ({
    contractVersion: 1,
    orderId,
    accepted: true,
    acceptedBy: state === "DUPLICATE"
      ? "WINERIM_IDEMPOTENCY"
      : "WINERIM_STOCK_READBACK",
    reason: state === "DUPLICATE"
      ? "existing_idempotent_mutation_certified"
      : execution?.reason ?? "absolute_stock_readback_certified",
    ...(execution?.attempts.at(-1)?.response?.status
      ? { responseStatus: execution.attempts.at(-1)!.response!.status }
      : {}),
    certifiedOrderIds: state === "DUPLICATE"
      ? [orderId]
      : [...(execution?.certifiedOrderIds ?? [])],
    stockDisposition,
  });
  if (state === "DUPLICATE") {
    return kind === "stock"
      ? { ok: true, duplicate: true, stockMoved: true, evidence: evidence("APPLIED_EXACT_ONCE") }
      : {
        ok: true,
        duplicate: true,
        lines: [{ duplicate: true }],
        evidence: evidence("APPLIED_EXACT_ONCE"),
      };
  }
  if (state === "APPLIED" && execution?.ok) {
    return kind === "stock"
      ? { ok: true, stockMoved: true, evidence: evidence("APPLIED_EXACT_ONCE") }
      : {
        ok: true,
        lines: [{ stockApplied: true }],
        evidence: evidence("APPLIED_EXACT_ONCE"),
      };
  }
...
function historicalImportFailureReason(execution: WinerimMutationExecutionResult): string {
  const response = execution.attempts.at(-1)?.response;
  const responseSample = diagnosticBodySample(response?.body);
  return [
    execution.reason,
    response ? `http=${response.status}` : null,
    responseSample ? `response=${responseSample}` : null,
...
function createSalesMutations(
  ...
  const importStockId = (line: SalesImportCommand["lines"][number]): number | null => {
    ...
    const mappings = Array.from(new Set(exactMappings.values())).filter((mapping) => (
  ...
  const applyStock = async (command: StockApplyCommand): Promise<StockApplyResult> => {
    ...
    return mapStockRun(result.state, result.execution, "stock", command.orderId) as StockApplyResult;
  };
  ...
  const importSales = async (command: SalesImportCommand): Promise<SalesImportResult> => {
    ...
    if (
      (command.live && command.stockDisposition !== "APPLIED_EXACT_ONCE")
      || (!command.live && command.stockDisposition === "APPLIED_EXACT_ONCE")
    ) {
      return { ok: false, status: 422, error: "SALES_IMPORT_STOCK_DISPOSITION_MISMATCH" };
    }
    ...
    if (!command.live) {
      const soldIdentity: WinerimStockIdentity = {
        wineId: line.winerimWineId,
        stockId: expectedStockId,
        variant: runtimeStockVariant(stockVariant(line.variant)),
      };
      const plan = {
        mode: "historical" as const,
        soldStock: soldIdentity,
        stockSource: soldIdentity,
        mutatesStock: false,
      ...
      if (execution.ok) {
        ...
        return {
          ...
          evidence: {
            ...
            orderId: command.orderId,
            accepted: true,
            acceptedBy: "WINERIM_MUTATION_RESPONSE",
            reason: execution.reason,
            ...(lastAttempt?.response?.status ? { responseStatus: lastAttempt.response.status } : {}),
            certifiedOrderIds: [...execution.certifiedOrderIds],
            stockDisposition: command.stockDisposition,
          },
        };
      }
      return {
        ...
        error: historicalImportFailureReason(execution),
      };
    }
    ...
    return mapStockRun(result.state, result.execution, "import", command.orderId) as SalesImportResult;
  };
...
async function legacyReceiptConflict(
  ...
  const result = await database.query<{ conflict_count: unknown }>(sql`
    ...
    WHERE connection_id = ${connectionId}::uuid
      ...
      AND NOT EXISTS (
        SELECT 1
        FROM public.runtime_idempotency owned
        WHERE owned.connection_id = receipts.connection_id
          AND owned.job = 'stock.mutation'
          AND owned.idempotency_key = receipts.idempotency_key
          AND owned.status = 'SUCCESS'
      )
      AND (
...
async function executeOpenTickets(
  ...
  const salesAdapter = createPostgresSalesAdapter(dependencies.database, {
    ...
    requireNativeSaleFormatIdentity: nativeSaleFormatIdentityRequired(connection),
  });
  const allIds = unique(documents.flatMap((document) => document.lines.flatMap((line) => [
    line.providerProductId,
    line.saleFormatId ?? "",
  ])));
  const mappings = await salesAdapter.readExactMappingsForLines(
    documents.flatMap((document) => document.lines),
  );
  const mappingById = exactSalesMappingIndex(mappings);
  ...
  const classified = planningDocuments(
    documents,
    mappingById,
    classifications,
    nativeSaleFormatIdentityRequired(connection),
  );
  ...
  const plan = await planSalesRun({
    ...
    resolveLine: async ({ line }) => exactSalesMappingForLine(mappingById, line, {
      requireNativePair: nativeSaleFormatIdentityRequired(connection),
    }),
  ...
  if (dryRun) {
    ...
    return {
      ...
      quarantinedClaimCount: plan.noops.filter((noop) => noop.reason === "CLAIM_QUARANTINED").length,
  ...
  if (plan.intents.length > 0 && await legacyReceiptConflict(
  ...
  return {
    ...
    blockedLineCount: plan.blocked.length,
    quarantinedClaimCount: plan.noops.filter((noop) => noop.reason === "CLAIM_QUARANTINED").length
      + execution.items.filter((item) => item.status === "QUARANTINED").length,
    executionCount: execution.items.length,
    dryRun: false,
...
async function executeDay(
  ...
  await salesAdapter.persistDocuments(
  ...
  const bounded = runKind === "CLOSED_DAY" && plan.blocked.length > 0
    ? {
      plan: { ...plan, intents: [] },
      pendingIntentCount: plan.intents.length,
    }
    : boundSalesExecutionPlan(plan);
  const executionPlan = bounded.plan;
  const boundedMutationKeys = unique(executionPlan.intents.map((intent) => intent.mutationIdempotencyKey));
  const boundedStockMutationKeys = unique(executionPlan.intents.flatMap((intent) => (
    intent.action.kind === "STOCK_APPLY" || intent.action.live
      ? [intent.mutationIdempotencyKey]
      : []
  )));
  if (executionPlan.intents.length > 0 && await legacyReceiptConflict(
    dependencies.database,
    connection.connectionId,
    unique(documents.map((document) => document.documentId)),
    boundedMutationKeys,
    unique(executionPlan.intents.flatMap((intent) => [intent.orderId, `${intent.orderId}:sales-only`])),
    unique(executionPlan.intents.map((intent) => legacyOrderPrefix(
      connection.connectionId,
      intent.businessDay,
      intent.winerimWineId,
      intent.variant,
  ...
  const mutations = createSalesMutations(dependencies, soldAtByOrderId(executionPlan), mappingById);
  const execution = await executeSalesPlan(executionPlan, {
    reserveClaim: salesAdapter.reserveClaim,
    completeClaim: salesAdapter.completeClaim,
    releaseClaim: salesAdapter.releaseClaim,
    ...mutations,
  ...
  const quarantinedClaimCount = plan.noops.filter((noop) => noop.reason === "CLAIM_QUARANTINED").length
    + execution.items.filter((item) => item.status === "QUARANTINED").length;
  let cursorAdvanced = false;
  if (
    runKind === "CLOSED_DAY"
    && bounded.pendingIntentCount === 0
  ...
  return {
    ...
    candidateLineCount: candidates.reduce((sum, document) => sum + document.lines.length, 0),
    executionCount: execution.items.length,
    pendingExecutionCount: bounded.pendingIntentCount,
    blockedLineCount: plan.blocked.length,
...
function failure(error: unknown): RuntimeExecutionResult {
  const laneError = error instanceof SalesLaneError
    ? error
    : new SalesLaneError("SALES_LANE_UNAVAILABLE", 503, true, unknownSalesDiagnostic(error));
  return {
    ok: false,
    failure: {
      ...
      ...(laneError.diagnostic ? { diagnostic: laneError.diagnostic } : {}),
    },
  };
}
...
export async function executeAgoraSalesEnvelope(
  ...
  try {
      ...
      return {
        ...
        detail: [
          ...
          ...(result.quarantinedClaimCount > 0
            ? [`quarantined:${result.quarantinedClaimCount}`]
            : []),
        ].join(":"),
      };
    }
    ...
    const pending = results.reduce((sum, result) => sum + result.pendingExecutionCount, 0);
    const blocked = results.reduce((sum, result) => sum + result.blockedLineCount, 0);
    const quarantined = results.reduce((sum, result) => sum + result.quarantinedClaimCount, 0);
    return {
      ok: true,
      detail: [
        `sales:${dryRun ? "dry-run" : "complete"}:${days.length}:${documents}:${items}`,
