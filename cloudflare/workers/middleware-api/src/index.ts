...
import { loadAgoraFleetReadModel } from "./agoraFleetReadModel";
...
async function handleAgoraFleet(request: Request, env: Env): Promise<Response> {
  ...
  if (unauthorized) return unauthorized;
  try {
    const readModel = await loadAgoraFleetReadModel(database(env));
    if (readModel.rows.length === 0) {
      return jsonResponse({
        success: false,
        error: "FLEET_READ_MODEL_EMPTY_OR_STALE",
      }, env, { status: 503 });
    }
    return jsonResponse({
      success: true,
      source: "agora_fleet_read_model",
      observedAt: readModel.observedAt,
      rows: readModel.rows,
    }, env);
  } catch (error) {
    return jsonResponse({
